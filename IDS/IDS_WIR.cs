            ...
            try
            {
                ...
                EFDbContext context = new EFDbContext(this.options);
                {
                    ...
                    if (wf != null)
                    {
                        if (vagons == null)
                        {
                            // Обновить организацию (погрузки выгрузки очистки)
                            if (mode == 8)
                            {
                                if (id_organization_service != null)
                                {
                                    ...
                                    foreach (WagonInternalMovement wim in wf.WagonInternalMovements)
                                    {
                                        ...
                                        if (wf.TypeFiling == 3 || wf.TypeFiling == 2 || wf.TypeFiling == 1)
                                        {
                                            if (wio != null)
                                            {
                                            ...
                                            {
                                                rt.SetModeResult(mode_obj.update, wim.Id, (int)errors_base.not_wio_db, wim.IdWagonInternalRoutesNavigation.Num); // В базе данных нет записи по WagonInternalOperation
                                            }
                                        }
                                    }
                                }
                                ...
                                {
                                    rt.SetResult((int)errors_base.error_input_value); // Нет данных по цеху
                                }
                            }
                            // Обновить цех (погрузки выгрузки очистки)
                            if (mode == 9)
                            {
                                if (id_division != null)
                                {
                                    wf.IdDivision = (int)id_division;
                                    wf.ChangeUser = user;
                                    wf.Change = DateTime.Now;
                                    context.WagonFilings.Update(wf);
                                    ...
                                    foreach (WagonInternalMovement wim in wf.WagonInternalMovements)
                                    {
                                        ...
                                        if (wf.TypeFiling == 2 || wf.TypeFiling == 1)
                                        {
                                            if (wimc != null)
                                            {
                        ...
                        if (wf.Close == null || mode == 9)
                        {
                            ...
                            foreach (WagonInternalMovement wim in wf.WagonInternalMovements.Where(m => m.FilingStart != null).ToList())
                            {
                                ...
                                if (wir != null)
                                {
                                    ...
                                    if (wf.TypeFiling == 2)
                                    {
                                        // определим груз текущий
                                        WagonInternalMoveCargo? wimc = wim.WagonInternalMoveCargoIdWimLoadNavigations.FirstOrDefault(w => w.IdWimLoad == wim.Id);
                                        // Проверим на смену типа погрузки
                                        if (id_wagon_operations != null)
                                        {
                                            WagonInternalOperation? wio = wim.IdWioNavigation;
                                            if (wio != null)
                                            {
                                                if (wio.OperationEnd == null)
                                                {
                                                    // Проверка меняем операцию
                                                    if (wio.IdOperation != id_wagon_operations)
                                                    {
                                                ...
                                                {
                                                    rt.SetErrorResult(wim.Id, (int)errors_base.wagon_close_operation, wir.Num); // операция закрыта
                                                }
                                            }
                                            else
                                            {
                                                rt.SetErrorResult(wim.Id, (int)errors_base.not_wio_db, wir.Num); // нет операции
                                            }
                                        }
