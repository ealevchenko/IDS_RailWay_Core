namespace IDS_
{
    ...
    public enum errors_base : int
    {
        ...
        not_input_value = -100,
        error_input_value = -101,
        not_input_list_wagons = -102,               // Ошибка, нет списка вагонов
        error_date = -103,                          // Ошибка, дата не прошла валидацию
        input_position_error = -104,                // Ошибка, позиция указана неправильно
        error_input_cargo = -105,                   // Ошибка, неправильно задан груз
        ...
        err_last_wim_db = -305,                     // Ошибка позиция вагона не соответствует последней позиции в базе
        wim_lock_wf = -306,                         // Вагон заблокирован, принадлежит другой подаче
        ...
        err_type_wf = -355,                         // Ошибка, тип подачи не соответствует операции
        err_wf_add_wagon = -356,                    // Ошибка, запрет добавления вагона в операцию
        err_wf_del_wagon = -357,                    // Ошибка, запрет удаления вагона из подачи (по вагону открыта следующая подача)
        ...
        wagon_not_operation = -403,                 // Операция вагона не соответствует выбранной
        ...
        err_last_wio_db = -408,                     // Ошибка, операция вагона несоответствие последней операции во внутренним перемещении
        ...
        cargo_not_load = -453,                      // Ошибка, вагон не погружен, выгрузка невозможна
        cargo_not_load_doc = -454,                  // Ошибка, вагон погружен, но без документа, выгрузка невозможна
        error_value_load_vz = -455,                 // Ошибка, неверный формат или не все параметры заданы для создания загрузки ВЗ
        error_value_load_uz = -456,                 // Ошибка, неверный формат или не все параметры заданы для создания загрузки УЗ
        error_value_operation = -457,               // Ошибка, неверный код операции
        error_update_load = -458,                   // Ошибка, обновления операции погрузки
        wimc_cargo_close = -459,                    // Ошибка, строка груза закрыта
        wimc_cargo_redirection = -460,              // Ошибка, груз переадресован
        wimc_update_lock = -461,                    // Обновление внутреннего груза - запрещено
        ...
        arrival_cars_num_main_doc = -509,          // По вагону не определен основной документ уз
        arrival_cars_num_doc = -510,               // По вагону неопределённа досылочных документ уз
        ...
        not_main_uz_doc_db = -514,                 // Ошибка в базе данных отсутствует основной ЭПД 
        not_uz_doc_db = -515,                      // Ошибка в базе данных отсутствует досылочных ЭПД 
        exist_not_manual_uz_doc_db = -516,         // Ошибка в базе данных указанный ЭПД введен не ручном режиме
        exist_not_manual_main_uz_doc_db = -517,    // Ошибка в базе данных указанный основной ЭПД введен не ручном режиме
        ...
        // документы по прибытию
        error_update_arr_doc_pay = -561,            // Ошибка обновления документов (плательщики)
        ...
        not_doc_uz_outgoing_cars = -607,            // В вагоне на отправку не определен id документа УЗ СМС
        ...
        not_out_uz_doc_db = -652,                   // В базе данных нет записи документа на отправку.
        not_out_uz_vag_db = -653,                   // В базе данных нет записи документа на вагон.
        error_out_uz_doc_not_code_payer = -654,     // В документе на отправку не указан плательщик 
        ...
        close_usage_fee_period = -752,                      // Ошибка, период уже закрыт
        ...
        not_usage_fee_period_detali_of_db = -771,           // Ошибка, в базе данных нет строки доп условия периода платы за пользования
        not_usage_fee_period_detali_error_data = -772,      // Ошибка данных доп условия (несовместимые или не корректные доп. условия) периода платы за пользования.
        ...
        // Плата за пользование -850..
        not_usage_fee_of_db = -850,                         // Ошибка, в базе данных нет строки рассчитанной платы за пользование
        ...
        // СМС   -1500
        error_connect_sms = -1500,                           // Ошибка Подключения к модулю согласования

        error_convert_epd = -1550,                           // Ошибка конвертации данных ЭПД
        not_epd_document = -1551,                            // Нет ЭПД
        not_vagon_epd_document = -1552,                      // Указанного вагона нет в ЭПД


        // Справочники -2000.....

        // Directory_Ways -2100..
        not_dir_way_of_db = -2101,                          // В базе данных нет записи указанной строки пути
        not_dir_park_station_of_db = -2102,                 // В базе данных нет записи указанного парка по указаной станции
        way_not_crossing_uz = -2103,                        // Путь не имеет выход на УЗ
        way_is_not_null = -2104,                            // На пути стоят вагоны
        way_is_close = -2105,                               // Путь закрыт
        way_is_delete = -2106,                              // Путь удален и не используется

        // Directory_ParkWays -2200..
        not_dir_park_of_db = -2201,                         // В базе данных нет указанного парка

        // Directory_Station -2300..
        ...
        exists_dir_wagon_of_db = -2404,                    // В базе данных уже есть запись по указанному вагону
        ...
        not_dir_outerways_of_db = -2501,                    // В базе данных нет записи указанного перегона
